...
class SliderConfig(BaseModel):
    ...
    type: Literal["int", "float"] = Field(
        ..., description=f"Numeric input type"
    )  # TODO Infer this from max and min
    min: Union[float, int] = Field(
        ..., description="Minimum allowed value", json_schema_extra={"type": "number"}
    )
    max: Union[float, int] = Field(
        ..., description="Maximum allowed value", json_schema_extra={"type": "number"}
    )
...
class Metric(BaseModel):
    ...
    description: Optional[str] = Field(
        default=None, description="Metric additional Information"
    )
    input: InputTypeKeys = Field(
        ...,
        description=f"Tracking UI element. Must be one of:\n{generate_enum_docs(InputTypeKeys)}",
    )
    config: Optional[Union[SliderConfig, FormConfig]] = Field(
        default=None, description="Config of slider or form input"
    )

    @model_validator(mode="after")
    def validate_config(self) -> Self:
        if self.input == InputTypeKeys.SLIDER.value and not isinstance(
            self.config, SliderConfig
        ):
            raise ValueError(
                f"Config for {self.input} must be of type {SliderConfig.__name__}."
            )
        elif self.input == InputTypeKeys.FORM.value and not isinstance(
...
class Habit(BaseModel):
    ...
    description: Optional[str] = Field(
        default=None, description="Habit additional Information"
    )
...
def validate_input_type_config(
    input_type: InputTypeKeys, config: Union[SliderConfig, FormConfig]
) -> None:
    input_rules = INPUT_VALIDATION_RULES.get(ActionKeys.CREATE.value, {}).get(
        input_type, {}
    )
    ...
    # Config-Input_type constraints checking
...
def validate_habit_metric_names(habit_name: str, metrics: List[Metric]) -> None:
    ...
    for metric in metrics:
        if (habit_name, metric.name) in context_manager.names_set:
            raise ValueError(
                f"Metric '{metric.name}' already exists for habit '{habit_name}'."
            )
    return None
